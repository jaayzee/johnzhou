...
const Contact: FC = () => {
  ...
  const getButtonContent = () => {
    const baseClasses = "absolute inset-0 flex items-center justify-center gap-2 transition-opacity duration-200";
    
    return (
      <>
        <span className={`${baseClasses} ${buttonState === 'idle' ? 'opacity-100' : 'opacity-0'}`}>
          Send Message
        </span>
        
        <span className={`${baseClasses} ${buttonState === 'loading' ? 'opacity-100' : 'opacity-0'}`}>
          <Loader2 className="h-5 w-5 animate-spin" />
          Sending...
        </span>
        
        <span className={`${baseClasses} ${buttonState === 'success' ? 'opacity-100' : 'opacity-0'}`}>
          <CheckCircle2 className="h-5 w-5" />
          Sent!
        </span>
        
        <span className={`${baseClasses} ${buttonState === 'error' ? 'opacity-100' : 'opacity-0'}`}>
  ...
  return (
    <main className="h-screen bg-background pt-16">
      <div className="container mx-auto px-4 py-16 h-full flex flex-col items-center">
        <div className="pt-10 bg-destructive rounded-3xl shadow-[0_0_15px_rgba(0,0,0,0.2)] h-fit">
          <h1 className="text-4xl px-20 text-center font-theme mb-12 text-foreground">Reach Me</h1>
          <form className="bg-muted rounded-b-3xl" onSubmit={handleSubmit(onSubmit)}>
            <div className='space-y-6 p-6'>
              <div>
                <input
                  type="email"
                  placeholder="Enter your email"
                  className="w-full font-bold px-4 py-2 bg-transparent border-b-2 border-foreground focus:border-b-4 focus:border-destructive rounded-none focus:outline-none transition-all"
                  required
                />
              </div>
              <div>
                <textarea
                  ...
                  rows={4}
                  className="w-full font-bold px-4 py-2 bg-transparent border-2 border-foreground rounded-md focus:border-4 focus:border-destructive focus:outline-none transition-all"
                  required
                ></textarea>
              </div>
            </div>
            <div className='bg-background-transparent space-y-4 py-6'>
              <div className='flex justify-center'>
                <button
                  type="submit"
                  disabled={buttonState !== 'idle'}
                  className={`
                    flex px-24 rounded-3xl font-bold relative h-12
                    transition-all duration-300
                    text-background
                    ${buttonState === 'idle' 
                      ? 'bg-foreground hover:bg-destructive'
                      : buttonState === 'loading'
              ...
              <div className="flex justify-center items-center space-x-6">
                <a href="https://www.linkedin.com/in/jjzee/" target="_blank">
                    <svg viewBox="0 0 448 512" className='w-6 h-6 text-foreground hover:text-destructive fill-current'>
