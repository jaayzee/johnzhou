...
export default function Model() {
  ...
  useEffect(() => {
    const updateColors = () => {
      const newStarColor = new THREE.Color(
    ...
    for (let j = -gridSize / 2; j < gridSize / 2; j++) {
      ...
      if (distFromCenter <= radius) {
        ...
        if (distFromCenter < fadeStartRadius) {
          ...
          const fadeProgress =
            (distFromCenter - fadeStartRadius) /
            (fadeEndRadius - fadeStartRadius);
  ...
  useFrame(({ clock }) => {
    ...
    const opacity = pointsRef.current.geometry.attributes.opacity
      .array as Float32Array;
    ...
    for (let i = 0; i < positions.length; i += 3) {
      ...
      if (dist <= radius) {
        ...
        const height =
          (mainWave + secondaryWave) * Math.max(0, 1 - (dist / radius) * 0.8) -
          edgeDroop;
        ...
        if (newDist < fadeStartRadius) {
          ...
          const fadeProgress =
            (newDist - fadeStartRadius) / (fadeEndRadius - fadeStartRadius);
          edgeOpacity = Math.max(0, 1 - Math.pow(fadeProgress, fadeExponent));
        }

        let valleyOpacity = 1.0;
        if (positions[i + 1] < valleyFadeThreshold) {
          valleyOpacity = Math.max(
            0,
            1 - (valleyFadeThreshold - positions[i + 1]) / valleyFadeRange
          );
        }
